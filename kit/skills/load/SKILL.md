...
- **Script-driven data gather.** The deterministic gather (read
  ...
  `bash <skill-dir>/load.sh ...` (not `./load.sh` — sync may not
  preserve the executable bit; `bash <script>` works regardless).
...
with `bash` explicitly (not `./load.sh`) — sync may not preserve
the executable bit, and the `bash <script>` form works regardless
of file mode.
...
- **Don't fall back to reading files directly.** All data gather
  goes through `load.sh orient`. Hand-reading bypasses the
  deterministic gather and risks divergence from /save's pattern.
