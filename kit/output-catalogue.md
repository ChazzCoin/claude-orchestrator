...
> **Kit-managed reference.** Generic across all orchestrator
> instances — synced via `/sync`. When writing a new orchestrator
> script (`bin/<name>`) or designing a skill's output, **pick a
> pattern from this catalogue** based on what the output is for.
> Don't invent custom shapes. The discipline that mandates this is
> in [`skill-architecture.md`](skill-architecture.md).
>
> **How to use this in practice.** Every script's header comment
> declares which catalogue pattern its output uses (e.g.,
> `# Output: Pattern 17 — Git branch overview`). Every skill's
> `SKILL.md` references the pattern when describing what the user
> will see. Consistency across scripts means a person who's read
> one orchestrator output knows how to read all the others.
>
> **High-relevance patterns for the orchestrator** (not exhaustive):
> 1 hero card · 2 live dashboard · 3 roadmap timeline ·
> 4 sprint board · 5 deployment report · 6 severity audit ·
> 17 git branch overview · 23 activity timeline · 25 alerts ·
> 26 empty state · 28 stats grid · 30 multi-step wizard ·
> 33 command reference · 34 selection prompt.
